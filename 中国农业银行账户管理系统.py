...
def useradd():
    ...
    type = input("请输入你的账户类型")
    print("下面请输入您的地址：")
    country = input("\t\t请输入你的国家")
    province = input("\t\t请输入您的省份")
    street = input("\t\t请输入您的街道")
    door = input("\t\t请输入您的门牌号")
    add = bank_add(account, username, password, type, country, province, street, door)
    if add == 3:
        print("数据库已满")
    elif add == 2:
        print("用户已存在")
        ...
        info = '''
                ...
                '''
        # 每个元素都可传入%
        print(info % (username, account, country, province, type, street, door, bank[username]["money"], bank_name))
